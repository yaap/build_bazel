...
def _clone(androidbp: Path, module: str, count: int):
    ...
    start_pattern = re.compile(r"^(?P<module_type>\w+)\s*\{\s*$")  # e.g. `cc_library {`
    nameline: int = -1
    with open(androidbp, "r+") as f:
        ...
        for i in range(1, count + 1):
            for j, line in enumerate(buffer):
                if j == nameline:
                    line = re.sub(
                        name_pattern, f"\\g<prefix>{module}-{i}\\g<suffix>", line
                    )
                f.write(line)
            f.write("\n")
...
def get_cuj_group(androidbp: Path, module: str) -> CujGroup:
    marker = uuid.uuid4()
    androidbp_bak: Final[Path] = util.get_out_dir().joinpath(f"android.bp.{marker}")
    allowlist_bak: Final[Path] = util.get_out_dir().joinpath(f"allowlists.go.{marker}")
    ...
    steps = [
        CujStep(verb=str(count), apply_change=functools.partial(helper, count))
        for count in counts
    ]
